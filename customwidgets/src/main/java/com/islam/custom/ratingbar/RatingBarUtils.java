package com.islam.custom.ratingbar;
...
class RatingBarUtils {
    ...
    static float calculateRating(PartialView partialView, float stepSize, float eventX) {
        ...
        Log.e("partialView","partialView: "+partialView.getTag());
        Log.e("partialView","partialView: "+partialView.getLayoutParams());
        Log.e("rating","calculateRating: "+ratioOfView);
        float steps = Math.round(ratioOfView / stepSize) * stepSize;

        Log.e("rating","calculateRating: "+steps);
        float f=Float.parseFloat(decimalFormat.format((int) partialView.getTag() - (1 - steps)));
