...
public class NotificationBadger extends Drawable {
    ...
    public NotificationBadger(Context context) {
        ...
        badgePaint1.setColor(ContextCompat.getColor(context.getApplicationContext(), R.color.colorNotification));
        badgePaint1.setAntiAlias(true);
        badgePaint1.setStyle(Paint.Style.FILL);

        badgePaint2 = new Paint();
        badgePaint2.setColor(ContextCompat.getColor(context.getApplicationContext(), R.color.colorNotification));
...
//          canvas.drawRoundRect(radius, radius, radius, radius, 10, 10, paint1);
    ...
    public static void setBadgeCount(Context context, LayerDrawable icon, String count,int drawableID) {

        NotificationBadger badge;

        // Reuse drawable if possible
        //Drawable reuse = icon.findDrawableByLayerId(R.id.ic_badge);
        Drawable reuse = icon.findDrawableByLayerId(drawableID);

        if (reuse instanceof NotificationBadger) {
            // Reuse Badge
        ...
        icon.setDrawableByLayerId(drawableID, badge);
    }
